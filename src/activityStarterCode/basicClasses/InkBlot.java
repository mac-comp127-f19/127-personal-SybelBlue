...
public class InkBlot {
     ...
     */
    public InkBlot() {
        canvas = new CanvasWindow("InkBlot", WIDTH, HEIGHT);
        canvas.getWindowFrame().setLocation(0,0);
    }
    ...
    public void generateInkBlot() {
        RandomWalk
                rx = new RandomWalk(WIDTH / 2, 0, WIDTH / 2),
                ry = new RandomWalk(HEIGHT / 2, 0, HEIGHT),
                red = new RandomWalk(125, 0, 255),
                green = new RandomWalk(125, 0, 255),
                blue = new RandomWalk(125, 0, 255);

        for (int i = 0; i < WIDTH * (HEIGHT / 4); i++) {
            Line basic = advance(rx, ry);
            basic.setStroked(true);
            basic.setStrokeColor(getColor(red, green, blue));
            basic.setStrokeWidth(3);
            canvas.add(basic);
            canvas.add(mirror(basic));
        }
    }
